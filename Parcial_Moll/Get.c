...
int GET_Sexo(char* msg, char* msgError, int reintentos, char resultado)
{
    int retorno = -1;
    char buffer;
    if(msg != NULL && msgError && reintentos>=0)
    {
        do
        {
            if(!GET_String(msg,msgError,1,2,reintentos,&buffer) && VAL_Sexo(buffer))
            {
                retorno = 0;
                break;
            }else
