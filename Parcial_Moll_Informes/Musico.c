...
int musico_modificar(Musico array[], int sizeArray)                                //cambiar musico
{
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        else
        {
            do
            {
                GET_Edad("'nModificar\n1-Edad\n2-Id de orquesta:\n3-Salir\nIngrese opcion:","Opcion Invalida",1,3,2,&opcion);
                switch(opcion)
                {
                case 1:
                    if(!GET_Edad("Ingrese la nueva edad","Edad invalida",10,88,2,&array[posicion].edad))
                    {
                        printf("\nEdad cambiada satisfactoriamente");
                    }
                    break;
                case 2:
                    if(!GET_Edad("Ingrese el nuevo id de la orquesta","Id invalid",0,QTY_ARRAY_ORQUESTA,2,&array[posicion].idOrquesta))
                    {
